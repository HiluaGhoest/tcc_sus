...
export default function ReagendarExame() {
  const location = useLocation();
  const navigate = useNavigate();
  const { exame, idx } = location.state || {};

  const [newDate, setNewDate] = useState(exame?.data || "");
  const [newTime, setNewTime] = useState(exame?.horario || "");
  ...
  useEffect(() => {
    ...
    const fetchMedicos = async () => {
      try {
        ...
        if (exame.unidade_cnes) {
          query = query.eq('unit_cnes', exame.unidade_cnes);
        } else if (exame.tipo) {
          // match specialty case-insensitive
          query = query.ilike('specialty', `%${exame.tipo}%`);
        }
        ...
        setMedicos(data || []);
        // set default selected medico to the one from exame if present
        const defaultMed = (data || []).find(m => m.id === exame.medico_id || m.nome === exame.medico);
  ...
  useEffect(() => {
    ...
    const medicosFiltrados = medicos.filter(m => {
      if (exame.unidade_cnes && m.unit_cnes !== exame.unidade_cnes) return false;
      if (exame.tipo && m.specialty && m.specialty.trim().toLowerCase() !== exame.tipo.trim().toLowerCase()) return false;
      return true;
    });
    let horarios = [];
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (exame.medico_id) {
        try {
          ...
          if (!prevMedError && prevMedData) {
            ...
            const filteredPrev = prevConsultas.filter(c => !(c.data === exame.data && c.horario === exame.horario && c.paciente_id === profileData.id));
            await supabase
              .from('logistica_medico')
      ...
      const targetMedico = selectedMedico || medicos.find(m => m.id === exame.medico_id) || null;
      if (targetMedico) {
        try {
          ...
          targetConsultas.push({ data: formatToStorageDate(newDate), horario: newTime, paciente_id: profileData.id, unidade: exame.unidade || null });
          await supabase
            .from('logistica_medico')
  ...
  <button type="button" onClick={() => navigate(-1)} className="text-gray-600 mb-6 flex items-center gap-2">◀ Voltar</button>
        ...
        <h1 className="text-2xl font-semibold mb-4">Reagendar Exame</h1>
        <p className="text-sm text-gray-500 mb-8">Selecione uma nova data e horário para seu exame</p>

  <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start max-w-7xl mx-auto">
          {/* Left column - current exame + info cards */}
          <aside className="lg:col-span-3 space-y-4">
            <div className="bg-white rounded-xl p-4 shadow-sm border">
              <div className="flex items-center gap-3">
                <img src={logo} alt="Dr" className="w-12 h-12 rounded object-contain bg-white p-1" />
                <div>
                  <div className="font-semibold text-gray-800">{exame?.medico || 'Dr. Nome'}</div>
                  <div className="text-sm text-gray-500">{exame?.tipo || 'Especialidade'}</div>
                </div>
              </div>
              ...
              <div className="bg-blue-50 border border-blue-100 rounded-xl p-4 text-sm text-blue-900">
                <div className="flex justify-between">
                  <div>
                    ...
                    <div className="font-medium">Médico: <span className="font-normal">{selectedMedico?.nome || exame?.medico || '—'}</span></div>
                  </div>
                </div>
              </div>
